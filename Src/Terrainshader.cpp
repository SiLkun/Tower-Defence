...
namespace TD
{
	...
	bool TerrainShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
	{
		...
		// Create the constant buffer pointer so we can access the pixel shader constant buffer from within this class.
		result = device->CreateBuffer(&textureInfoBufferDesc, NULL, &pTextureInfoBuffer);
		if(FAILED(result))
		{
			return false;
		}
	...
	bool TerrainShader::SetShaderTextures(ID3D11DeviceContext* deviceContext, ID3D11ShaderResourceView* texture1, 
											   ID3D11ShaderResourceView* texture2, ID3D11ShaderResourceView* alphaMap, bool useAlpha)
	{
		...
		result = deviceContext->Map(pTextureInfoBuffer, 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
		if(FAILED(result))
		{
			return false;
		}
		...
		// Unlock the constant buffer.
		deviceContext->Unmap(pTextureInfoBuffer, 0);
		...
		deviceContext->PSSetConstantBuffers(bufferNumber, 1, &pTextureInfoBuffer);
