...
namespace TD
{
	...
	void Terrain::Render(ID3D11DeviceContext* pDeviceContext)
	{
	...
	bool Terrain::InitializeBuffers(ID3D11Device* pDevice)
	{
		VertexType* vertices;
		...
		result = pDevice->CreateBuffer(&vertexBufferDesc, &vertexData, &pVertexBuffer);
		if(FAILED(result))
		{
			return false;
		}
		...
		result = pDevice->CreateBuffer(&indexBufferDesc, &indexData, &pIndexBuffer);
		if(FAILED(result))
		{
			return false;
		}
	...
	void Terrain::RenderBuffers(ID3D11DeviceContext* pDeviceContext)
	{
		unsigned int stride;
		unsigned int offset;
