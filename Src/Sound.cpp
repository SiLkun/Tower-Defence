...
namespace TD
{
	...
	bool Sound::Initialize(char* filename,HWND hwnd)
	{
		bool result;
		...
		result = LoadWaveFile(filename, &pSecondaryBuffer,&pSecondary3DBuffer);
		if(!result)
		{
			return false;
		}
	...
	bool Sound::InitializeDirectSound(HWND hwnd)
	{
		...
		pListener->SetPosition(0.0f, 0.0f, 0.0f, DS3D_IMMEDIATE);
	...
	bool Sound::LoadWaveFile(char* filename, IDirectSoundBuffer8** secondaryBuffer, IDirectSound3DBuffer8** secondary3DBuffer)
{
	...
	void Sound::Update(D3DXVECTOR3 p)
	{
		pListener->SetPosition(p.x, p.y, p.z, DS3D_IMMEDIATE);
	}

	bool Sound::Play(D3DXVECTOR3 p)
	{
		HRESULT result;


		// Set position at the beginning of the sound buffer.
		result = pSecondaryBuffer->SetCurrentPosition(0);
		if(FAILED(result))
		{
			return false;
		}
		...
		result = pSecondaryBuffer->SetVolume(DSBVOLUME_MAX);
		if(FAILED(result))
		{
			return false;
		}
		...
		pSecondary3DBuffer->SetPosition(p.x, p.y, p.z, DS3D_IMMEDIATE);

		// Play the contents of the secondary sound buffer.
		result = pSecondaryBuffer->Play(0, 0, 0);
		if(FAILED(result))
		{
			return false;
		}
